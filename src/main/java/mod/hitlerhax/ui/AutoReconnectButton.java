...
public class AutoReconnectButton extends GuiButton {
	public AutoReconnectButton(int buttonId, int x, int y, String buttonText) {
		...
		reconnectTimer = ((IntSetting) Main.settingManager.getSettingByName(Main.moduleManager.getModule("Reconnect"),
				"Timer")).value;
	}
	...
	public void drawButton(Minecraft mc, int mouseX, int mouseY, float partialTicks) {
		...
		if (visible && ((Reconnect) Main.moduleManager.getModule("Reconnect")).serverData != null) {
			...
			if (Mod.toggled && timer.passed() < reconnectTimer)
				this.displayString = "AutoReconnect " + (reconnectTimer - timer.passed());
			else if (!Mod.toggled)
			...
			if (timer.passed() + 100 >= reconnectTimer && Mod.toggled
